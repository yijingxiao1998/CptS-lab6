...
int mymkdir(MINODE* pip, char* child)
{
   ...
   ino = ialloc(dev);   // allocate inode number from bitmap
   blk = balloc(dev);   // return free disk block number
   printf("ino = %d, blk = %d\n", ino, blk);
   ...
   get_block(dev, blk, buf);  // get block into buffer
   DIR *dp = (DIR *)buf;
   ...
   dp = (char *)dp + 12;  // advance to next block
   dp->inode = pip->ino;
