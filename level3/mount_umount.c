...
int mount(char* filesys, char* mount_point)   /*  Usage: mount filesys mount_point OR mount */
{
   ...
   for(i = 0; i < NMTABLE; i++)
   {
   	if(mtable[i].dev)
   	{
         // If already mounted, reject;
   	   if(strcmp(mtable[i].devName, filesys) == 0 || strcmp(mtable[i].mntName, mount_point) == 0)
   	   {
   ...
   //Open the filesys virtual disk (under Linux) for RW; use (Linux) file
  ...
  printf("EXT2 FS OK\n");
   ...
   ino = getino(mount_point); // pass &dev as an extra parameter
   if(ino == 0)
   {
   ...
   printf("Recording mntDirPtr\n");
   mountptr->mntDirPtr = iget(fd, 2);
   printf("Recording dev\n");
   mountptr->dev = fd;
   printf("Recording ninodes\n");
   mountptr->ninodes = ssp->s_inodes_count;  // copy super block info into mtable
   printf("Recording nblocks\n");
   ...
   printf("Recording mounted\n");
   mountptr->mntDirPtr->mounted = 1;
   //mountptr->mntDirPtr = iget(dev, 2);
